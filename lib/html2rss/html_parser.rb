...
module Html2rss
  ##
  # Provides an abstraction layer around the HTML/XML parsing backend.
  #
  ...
  class HtmlParser
    ...
    class << self
      ...
      # @return [Object]
      def parse_xml(xml)
        backend.parse_xml(xml)
      end

      ##
      # Parses an XML fragment string.
      #
      # @param xml [String]
      # @return [Object]
      def parse_xml_fragment(xml)
        backend.parse_xml_fragment(xml)
      end
      ...
      ##
      # @return [Class] backend specific XML document class
      def xml_document_class
        backend.xml_document_class
      end
