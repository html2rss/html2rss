...
`Html2rss.capture` (and CLI `html2rss capture`) analyzes a URL through the feed pipeline and produces a reusable config with **items selector + `enhance: true` only** — no title/url/description attribute-selector soup. At feed-build time, `enhance: true` fills missing article fields via `Html::ArticleExtractor` on each **matched list-card item node** — it never fetches per-item detail pages. Future `enhance_detail` (detail-page fetch per item) is documented in schema/playbook only; not implemented.
