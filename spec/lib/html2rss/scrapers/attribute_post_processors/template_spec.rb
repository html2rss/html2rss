...
RSpec.describe Html2rss::Scrapers::AttributePostProcessors::Template do
  subject { described_class.new('Hi', options:, item:, scraper:).get }

  let(:item) { Object.new }
  let(:scraper) { instance_double(Html2rss::Scrapers::Selectors) }
  ...
  context 'when the string is empty' do
    it 'raises an error' do
      expect do
        described_class.new('', {})
      end.to raise_error(Html2rss::Scrapers::AttributePostProcessors::InvalidType, 'The `string` template is absent.')
    end
  end
