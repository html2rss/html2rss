  ...
  context 'when the budget is exhausted' do
    ...
    it 'stops pagination and logs the stop reason', :aggregate_failures do
      ...
      expect(logger).to have_received(:warn).with(
        %r{Html2rss::RequestSession::RelNextPager: pagination stopped at https://redirected\.example\.com/news\?page=2 - Request budget exhausted\. Retry with --max-requests 4 or increase request.max_requests in the config\.}
      )
    end
  end
end
