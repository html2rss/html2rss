...
RSpec.describe Html2rss::Selectors::AttributePostProcessors do
  ...
  describe '.get_processor' do
    it { expect(described_class.get_processor(:gsub)).to be(Html2rss::Selectors::AttributePostProcessors::Gsub) }

    it {
      expect do
        described_class.get_processor(:html_to_mark)
      end.to raise_error(Html2rss::Selectors::AttributePostProcessors::UnknownPostProcessorName)
    }
  end
end
