...
RSpec.describe Html2rss::SST::Attrs do
  describe '.build' do
    ...
    it 'exposes typed [] lookups and class_attr', :aggregate_failures do # rubocop:disable RSpec/ExampleLength
      ...
      expect(attrs['class']).to eq('one two')
      ...
      expect(attrs['data-category']).to eq('News')
