...
RSpec.describe Html2rss::AttributePostProcessors do
  ...
  describe '.get_processor' do
    it { expect(described_class.get_processor(:gsub)).to be(Html2rss::AttributePostProcessors::Gsub) }

    it {
      expect do
        described_class.get_processor(:html_to_mark)
      end.to raise_error(Html2rss::AttributePostProcessors::UnknownPostProcessorName)
    }
  end
end
